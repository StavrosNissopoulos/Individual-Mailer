...
public class UserC extends User {
	...
	public UserC(int id, String usename, String passw, int type, Database myDb) {
		...
		if (titles.size() != 0) {
			...
			if (messageSelected != 0) {
				...
				do {
					...
					System.out.println("Press 3 to reply to this message");
					System.out.println("Press 0 to go to the previous menu\n");
					choice = getMessageInputZero(3);
					if (choice == 1) {
	...
	// user c executes a change of state of the message and admin can do the real
	// delete
	private void deleteInbox(int messageId) {
		myDb.changeMessageState(messageId, 0);
		System.out.println("\n---------------------------");
